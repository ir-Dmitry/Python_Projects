...
async def send_file_section(
    message: types.Message, filename: str, section: int = 1, parse_mode=ParseMode
):
...
async def send_commands_list(message: types.Message):
    ...
    for (
        ...
        try:
            text += f": {escape_specific_markdown(command_details['description'])}\n"
        except:
            text += ": description не указан"
